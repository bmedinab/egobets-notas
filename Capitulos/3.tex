...
\paragraph{Ejemplificando} % (fold)
\label{par:ejemplificando}
...
\paragraph{Aplicando Kelly a las apuestas de futbol} % (fold)
\label{par:aplicando_kelly_a_las_apuestas_de_futbol}
El criterio de Kelly se puede extender a las apuestas de futbol. Retómese la situación original pero ahora el jugador puede ganar $b$ pesos por cada peso invertido en la apuesta. Además, considérese que el jugador no apostará a juegos no favorables, es decir que sólo apostará cuando la probabilidad de ganar $p>0$ sea ventajosa, i.e. $pb - q > 0$.
...
\[g(f) = E(log(\frac{X_n}{X_0}))=p\log(1+bf)+q\log(1-f)\]
Y se obtiene que:
\[f^* = \frac{bp -q}{b}\]
