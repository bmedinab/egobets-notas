...
CodeIgniter es un framework\footnote{Es una estructura de software compuesta de componentes personalizables e intercambiables para el desarrollo de una aplicación. En otras palabras, un framework se puede considerar como una aplicación genérica incompleta y configurable a la que se le puede añadir las últimas piezas para construir una aplicación concreta.} de PHP que ahorra tiempo en le programación, robustece tu sistema y permite al programador alcanzar un grado mayor de sofisticación en su código. 
	...
	Cómo se puede observar en la figura ~\ref{Fig:db-egobets}, se cuenta con seis clases principales:
	\begin{itemize}
		...
		\item Administrador. Estos usuarios son los que tienen privilegios para ingresar al portal administrador (Ver \ref{sec:inserting-data}) y gestionar la información del sistema.
		...
		\item Casa. Guarda las casas de apuestas y su URL.
		\item Resultado. Se tiene la cantidad de dinero que obtiene o pierde un usuario en una jornada.
	\end{itemize}
	Cómo se puede observar, hay conexiones entre algunas clases. Esto se debe a que las colecciones que tiene MongoDB permiten objetos complejos, de tal manera que un objeto puede contener un arreglo de otros objetos. Para ver un ejemplo de los datos almacenados en la base de datos vease \ref{chap:dbs}.
	
	Con base en la estructura de datos definida, retómese el punto más interesantes de CodeIgniter, el patrón de diseño MVC. Este patrón fue descrito por el noruego Trygve Reenskaug en 197, retomando este concepto junto con Upton \cite{upton2007codeigniter} y  Weitzenfeld \cite{alfredo2005ingenieria} se puede describir el sistema de Egobets de una manera más organizada:
	
	\begin{itemize}
		\item \textbf{Modelos.} La información representa el dominio del problema y la base de datos se abstrae como un conjunto de objetos sobre los cuales recaen todas la acciones. Estos objetos reflejan las tabla de la base de datos y pueden modificarla conforme sea requerido. Los modelos también realizan operaciones a los datos según sea necesario.
		En particular, es importante notar que la estructura de datos definida en la figura ~\ref{Fig:db-egobets} representa de igual manera los objetos definidos en el código definido en los modelos del sistema
		\item \textbf{Vistas.} reflejan el estado del modelo. Son las responsables de desplegar la información al usuario final. En este caso específico, todas las vistas son representaciones HTML del contenido.
